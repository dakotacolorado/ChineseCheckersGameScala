...
class Hexagram(radius: Int = 4) {
  require(0 < radius)
  /**
   * The hexagon points are the 6 corner points of a hexagon, written in the following 2d basis:
   * <ul>
   *  <li>u1 = [1, 0]</li>
   *  <li>u2 = [1/2, Sqrt(3)/2]</li>
   * </ul>
   */
  final val hexagonPoints: Array[(Int, Int)] = Array(
    (1, 0), (0, 1), (-1, 1), (-1, 0), (0, -1), (1, -1)
  )

  /**
   * The hexagon edges are the lines between two neighbor points in a hexagon.
   */
  private final val hexagonEdges: Array[((Int, Int), (Int, Int))] = this.hexagonPoints.zipWithIndex.map(
    (point: ((Int, Int), Int)) => (
      point._1,
      this.hexagonPoints((point._2 + 1) % 6)
    )
  )

  /**
   * The rhombus grid is a square lattice of points with height/width equal to the length is the hexagram's inner radius.
   */
   ...
   */
  //noinspection MapFlatten
  final val hexagramPoints: Array[(Int, Int)] = this.hexagonEdges.map(
   ...
   */
  //noinspection MapFlatten
  final val hexagramCornerPoints: Array[Array[(Int, Int)]] = this.hexagonEdges
    .map(
